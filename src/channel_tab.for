...
!K2_PROFILER
      if (profiler) then
        ...
        write(789, '(f8.2,100(",",f8.2))') fav(1,1)*1000.*3600.,
     &                                 (fav(j,1)*1000.*3600, j = 2, nk)
        if (sed) then
          do n = 1, nps
            write(789, '(f10.6,100(",",f10.6))')
     &        conc2(n,1)*q2(1,1)*rho(n)*1000.,
     &       (conc2(n,j)*q2(j,1)*rho(n)*1000., j = 2, nk)
          end do
          do n = 1, nps
...
100   continue
      ...
      if(diag .and. cour) then
        write(99,'(/I4," resets of dt by courant criteria. ")') iretn
      end if
C
      if(vtn .gt. 0.) then
       avwr = swr/vtn
